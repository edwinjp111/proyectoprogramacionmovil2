...
// erroe
//el 25 era un numero no un string entonces por es nos arrojaba el error el cual se soluciono 
// se pasaba el valor a texti antes del .replace
function sanitizeInput(input) {
    ...
    return String(input).replace(/[&<>"'/]/g, (match) => characterMap[match]);
}
...
async function fetchPokemon(query) {
    ...
    // error
    // Lo que sucede es que la pagina normal funciona con https la cual es segura 
    //y al poner http hace como un mixed y a lo cual la pagina se bloquea por que no es seguro
...
function renderCard(data) {
    ...
    // error:
    // este error fue por que el stants se estaba utilizando como un objeto pero en realidad 
    // es un arreglador mas no un objeto
    const getStat = (statName) => {
        const stat = data.stats.find(
            (statInfo) => statInfo.stat.name === statName
        );

        return stat ? stat.base_stat : "N/A";
    };

    attackStat.textContent = getStat("attack");
    defenseStat.textContent = getStat("defense");
    speedStat.textContent = getStat("speed");
